...
Let's give `read_csv()`, "a special [case] of the more general `read_demin()` a go. We just need to provide the file path from our working directory to the file and pass that into the first parameter (`file`) in `read_csv()`.

```{r}
read_csv("2022-03-28/simple.csv")
```
...
```{r}
df_months <- read_csv("2022-03-28/simple.csv")
```
### Export
...
```{r eval=F}
write_csv(df_months, "2022-03-28/simple_out.csv", na = "")
```
...
```{r}
read_csv("2022-03-28/missing.csv", guess_max = 5)
...
excel_sheets("2022-03-28/hfr.xlsx") %>% 
  str_subset("HFR") %>% 
  map_dfr(.f = ~read_excel("2022-03-28/hfr.xlsx", 
                                  sheet = .x, 
                                  skip = 1))
```
...
Problem: Using what you’ve learned, import in all the [data from the HIV estimates](https://github.com/USAID-OHA-SI/coRps/raw/main/2022-03-28/unaids_more_messy.xlsx) to create a tidy dataset.
...
xl_path <- "2022-03-28/unaids_more_messy.xlsx"
