...
```{r}
  ...
  GET(url = "https://datim.org")
  
  GET(url = "https://www.datim.org/api/resources")
...
```{r}
  ...
  res_url <- "https://www.datim.org/api/resources"
...
```{r}
  ...
  1:10 %>% map(~get_ouorglabel(operatingunit = cntry, org_level = .x)) %>% unlist()
...
DHIS leverage `dimensions` to build complex datasets. Let's look at what DATIM has to offer.
