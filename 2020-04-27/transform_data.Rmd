...
A very simple but powerful function to use with `mutate` is `ifelse()`. With `ifelse()`, you first specify a logical statement, afterwards what needs to happen if the statement returns TRUE, and lastly what needs to happen if it’s FALSE.
...
df %>% 
  mutate(fundingagency = ifelse(fundingagency == "USAID", "USAID", "Other")) %>% 
  count(fundingagency)
