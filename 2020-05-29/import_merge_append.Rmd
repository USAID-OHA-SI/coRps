...
2. glamr
This package is a syster package of ICPIutilities for working with data from datim and the MER
...
```{r}
here("2020-05-29", "HIV_estimates_from_1990-to-present.xlsx")
```
...
Let's try to read the content of the excel file again

```{r}
df <- read_xlsx(path = here("2020-05-29", "HIV_estimates_from_1990-to-present.xlsx"), 
              sheet = "Treatment - by Year",
              skip=5)
```
...
There is a quick way of taking a quick pick at excel file content
Let's list out the names of all tabs in this file

```{r}
excel_sheets(path = here("2020-05-29", "HIV_estimates_from_1990-to-present.xlsx"))
```
...
```{r}
path <- here("2020-05-29", "HIV_estimates_from_1990-to-present.xlsx")
