...
library(usethis)
getwd()
project_path <- c("/Users/tim/Documents/MyNewProject")
...
datim_path <- "/Users/tim/Documents/DATIM_DATA"
...
list.files(path = datim_path, pattern = "Zambia")
...
df_long %>% 
  ...
  count(indicator, period) %>% prinf()

# Let's make a table of the counts across periods, can use str_detect to help us
df_long %>% 
...
df_long %>% 
  ...
  group_by(period) %>% 
  summarise(TX_CURR = sum(val, na.rm = TRUE)) %>% 
  ggplot(aes(x = period, y = TX_CURR)) + geom_col() +
